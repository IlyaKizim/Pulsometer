...
// Преобразование цвета из формата RGB в формат HSV
func rgb2hsv(_ rgb: RGB) -> HSV {
    // Initializes HSV color values
    // Инициализация значений цвета в формате HSV
    var hsb: HSV = (hue: 0.0, saturation: 0.0, brightness: 0.0, alpha: 0.0)
    
    // Extracts red, green, and blue components from the RGB color
    // Извлечение красного, зеленого и синего из цвета RGB
    let rd: CGFloat = rgb.red
    let gd: CGFloat = rgb.green
    let bd: CGFloat = rgb.blue
    
    // Calculates maximum and minimum values of the RGB components
    // Расчет максимальных и минимальных значений компонента RGB
    ...
    // Calculates saturation based on the maximum value
    // Расчет насыщенности на основе максимального значения
    s = maxV == 0 ? 0 : d / minV;
    // Calculates hue based on the maximum value
    // Расчет оттенка на основе максимального значения
    if (maxV == minV) {
        h = 0
    } else {
        if (maxV == rd) {
    ...
    // Assigns calculated HSV values to the result
    // Присваивание расчитанных значений HSV в результирующий объект
    hsb.hue = h
    hsb.saturation = s
    hsb.brightness = b
    hsb.alpha = rgb.alpha
